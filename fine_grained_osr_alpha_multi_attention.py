...
class MultiHeadCrossAttention(nn.Module):
    """多头交叉注意力机制"""
    def __init__(self, feature_dim: int = 128, num_heads: int = 8, dropout: float = 0.1):
        ...
        # Query, Key, Value projections for feat1
        self.q_proj = nn.Linear(feature_dim, feature_dim)
        # Key, Value projections for feat2 (cross-attention)
    ...
    def forward(self, feat1: torch.Tensor, feat2: torch.Tensor) -> torch.Tensor:
        ...
        多头交叉注意力融合
        Args:
            feat1: [B, feature_dim] - temporal_features (作为Query)
            feat2: [B, feature_dim] - combined_features (作为Key和Value)
        Returns:
        ...
        # 将单个向量视为长度为1的序列，以便使用标准的注意力机制
        # [B, feature_dim] -> [B, 1, feature_dim]
        feat1_seq = feat1_norm.unsqueeze(1)  # [B, 1, feature_dim]
        feat2_seq = feat2_norm.unsqueeze(1)  # [B, 1, feature_dim]
        
        # Project to Q, K, V
        Q = self.q_proj(feat1_seq)  # [B, 1, feature_dim]
        K = self.k_proj(feat2_seq)  # [B, 1, feature_dim]
        V = self.v_proj(feat2_seq)  # [B, 1, feature_dim]
        
        # Reshape for multi-head attention: [B, 1, feature_dim] -> [B, 1, num_heads, head_dim] -> [B, num_heads, 1, head_dim]
        ...
        scores = torch.matmul(Q, K.transpose(-2, -1)) * self.scale  # [B, num_heads, 1, 1]
        ...
        attn_output = attn_output.view(B, 1, self.feature_dim)  # [B, 1, feature_dim]
        ...
        out = out + self.ffn(self.norm1(out))
        
        return out


class AttentionFusion(nn.Module):
    """强大的多头交叉注意力融合模块，用于融合两个相同维度的特征"""
    def __init__(self, feature_dim: int = 128, num_heads: int = 8, dropout: float = 0.1):
        ...
        # 双向交叉注意力（feat2作为Query，feat1作为Key/Value）
        self.reverse_cross_attention = MultiHeadCrossAttention(
            feature_dim=feature_dim,
            num_heads=num_heads,
            dropout=dropout
        )
        
        # 门控机制，决定两个交叉注意力输出的融合比例
    ...
    def forward(self, feat1: torch.Tensor, feat2: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            fused_feature: [B, feature_dim]
        """
        # 第一个交叉注意力：feat1作为Query，feat2作为Key/Value
        attn_out1 = self.cross_attention(feat1, feat2)  # [B, feature_dim]
        
        # 第二个交叉注意力：feat2作为Query，feat1作为Key/Value
        attn_out2 = self.reverse_cross_attention(feat2, feat1)  # [B, feature_dim]
        
        # 拼接两个注意力输出用于门控机制
        concat_attn = torch.cat([attn_out1, attn_out2], dim=-1)  # [B, 2*feature_dim]
        
        # 门控机制决定融合比例
        gate_weights = self.gate(concat_attn)  # [B, 2]
        gated_out = gate_weights[:, 0:1] * attn_out1 + gate_weights[:, 1:2] * attn_out2  # [B, feature_dim]
        
        # 最终投影和融合：将门控输出和原始拼接特征结合
        final_input = torch.cat([gated_out, attn_out1 + attn_out2], dim=-1)  # [B, 2*feature_dim]
        fused = self.final_proj(final_input)  # [B, feature_dim]
