...
from openood.networks.resnet18_32x32_512 import ResNet18_32x32
...
class Config_gnn:
    def __init__(self):
        ...
        self.save_dir = "results/fine_grained_baseline"
        self.device = device
...
class FCTFNet(nn.Module):
    def __init__(self, num_classes: int = 8, signal_length: int = 128,
        ...
        #etf
        self.etf_classifier = ETF_Classifier(feat_in=512, num_classes=num_classes)
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        ...
        # logits_patch = F.softmax(self.etf_classifier(temporal_features), dim=1)
        logits_combined = self.etf_classifier(combined_features)
        # logits_patch = self.etf_classifier(temporal_features)
        # fusion_weights = torch.sigmoid(self.fusion_weights).unsqueeze(0)
        # logits = fusion_weights * logits_combined + (1 - fusion_weights) * logits_patch
...
def plot_confusion_matrix(y_true, y_pred, class_names=None, title='Confusion Matrix', 
    ...
    # 如果需要归一化，创建归一化版本用于可视化
    if normalize:
        # 按行归一化：每行除以该行的和（每行和为1）
        row_sums = cm_raw.sum(axis=1, keepdims=True)  # 保持维度用于广播
        # 避免除零：如果某行为0，则保持为0
        cm_display = np.divide(cm_raw.astype('float'), row_sums, 
    ...
    for i, j in itertools.product(range(cm_display.shape[0]), range(cm_display.shape[1])):
        ...
        if normalize:
            if cm_display[i, j] > 0:
                text = f'{cm_display[i, j]:.1%}'  # 显示为百分比，如 85.3%
            else:
                text = '0%'
        else:
            text = f'{int(cm_display[i, j])}'  # 显示为整数
        plt.text(j, i, text,
    ...
    # 重要：使用原始混淆矩阵计算所有指标
    # 注意：归一化后的矩阵每行和为1，列和不为1，不能用于计算指标
    # cm_raw: 原始混淆矩阵（未归一化）
    # cm_display: 归一化后的矩阵（仅用于可视化）
    
    # 准确率 = 对角线元素之和 / 总样本数
    accuracy = np.trace(cm_raw) / np.sum(cm_raw)
    print(f"总体准确率: {accuracy:.4f}")
    
    # 计算每个类别的精确率、召回率和F1分数（使用原始矩阵）
    # 精确率 = TP / (TP + FP) = 对角线元素 / 列和（预测为该类的样本数）
    precision = np.diag(cm_raw) / np.sum(cm_raw, axis=0)
    # 召回率 = TP / (TP + FN) = 对角线元素 / 行和（真实为该类的样本数）
...
def test_and_plot_confusion_matrix(checkpoint_path=None, class_names=None, normalize=True):
    ...
    if checkpoint_path is None:
        checkpoint_path = os.path.join(config_gnn.save_dir, 'best_epoch86_acc0.8139.ckpt')
